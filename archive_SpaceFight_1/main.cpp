...
HBITMAP bg, guiBG;		//hintergrund + lebensleiste
HBITMAP livePic[10];	//zahlen für lebensanzeige
int first;
int i;					//integer für for-schleifen
int eclapsed;			//pseudo timer
int lives100;			//benötigt für lebensanzeige
int lives10;			//ebenso
int lives1;				//ebenso
int tempint;			//integer für for-schleifen (ka ob des no verwendet wiad)
int maxlive = 200;		//startleben
int msgbox1;			//message box (sieger etc)
//long sek;
//time(&sek);
//srand((unsigned)sek);
...
//	es folgt wos wou i no ned eibaut ho und zwar das des plasma ding lila boppln hinterlousd wou kuhl aussschaua
/*class DROP
{
public:
	RECT rct;
...
class SHIELD				//klasse für schild
{
public:
	RECT rct;				//rechteck zum löschen
	HBITMAP hbm;			//graphik
	HBITMAP mask;			//mask
	BITMAP bmp;				//bmp... beinhaltet eig blos höhe und breite, bild selber is in HBITMAP
	//int X;
    //int Y;
	float modifier;			//schadensreduktion bzw steigerung
	int on;					//schild da?
	int time;				//pseudo timer
	int reflect;			//reflektiert da schild?
	void update()
	{
		if(time == 1)		//schild wird zerstört
		{
			//X = 2000;
			//Y = 2000;
			time = 0;
		...
		else if(time > 1)	//pseudo timer wird verringert und schild wird beim ersten durchlauf aktiviert
		{
			time--;
			on = 1;
		}
...
class projectile			//klasse für schüsse
{    
	public:
	HDC hdc;
	RECT rct;
    ...
    int Y;					//Y-Koordinate
    int A;					//Winkel
    int delta;				//bewegungsgeschwindigkeit in pixel/frame
    int dmg;				//schaden
    int owner;				//selbsterklärend
	int alive;				//selbsterklärend
	int height;				//höhe des bmp
	int width;				//breite
	int follow;				//lenkrakete?
	int type;				//typ
	int slow;				//verlangsamung
	int slowTime;			//pseudo timer
	SHIELD addShield;		//hinzugefügter schild (frostschuss)
	int anim;				//animationsframes (plasma)
	int animtemp;			//temporärer integer für animationsschleife
	HBITMAP frame[20];		//animationsframes
	int framedelay;			//pseudo timer für zeit zwischen frames
	int framedelaytemp;		//temporärer integer für animationsschleife
	/*DROP drop[50];
	int dropnum;
	int dropdelay;
	int dropdelaytemp;*/
...
class player				//klasse für spieler
{
public:
	RECT rct;
	HBITMAP hbm;
	HBITMAP liveBmp[3];		//graphiken für lebensanzeige
	HBITMAP mask;
	BITMAP bmp;
	int X;
    int Y;
    int lives;				//leben
    int A;
    int skill;				//nicht benutzt
    int wpn;				//derzeitige waffe
    int delta;
    int pid;				//ID
	int down;
	int speedbuffremove;	//pseudo timer
	int slowhealremove;		//ebenso
	float slowheal;			//ebenso
	int delay;
	float dmgModifier;		//schadensmultiplikator (zB bei doppelter schaden buff benutzt)
	int dmgModRemove;		//pseudo timer
	SHIELD shield;			//derzeitiger schild
};

class buff			//klasse für buffs
{
public:
	int type;
/*	1 = heal
	...
	void create(HDC gethdc)
	{
		...
		switch(type)						//graphik hinzufügen
		{
		case 1:
...
int getdistance()
{
	int ret = (sqrt(((P2.X - P1.X) * (P2.X - P1.X)) + ((P2.Y - P1.Y) * (P2.Y - P1.Y))));
	if(ret < 0)
	{
		ret = -ret;
	}
...
void shoot(int X, int Y, int A, int owner, int wpn, HDC hdc)		//funktion für schießen
{
     projectile tempProj;
     tempProj.X				= X;
     tempProj.Y				= Y - 16;
...
void buff::update()	//schauen ob spieler in buff geflogen ist
	{
		if(alive == 1)
		{
			rct.left = X;
...
void buff::use(int Player)		//buff benutzen
	{
		X = -50;
		if(Player == 1)
		{
...
void projectile::update()		//projektil bewegen und treffer üebrprüfen
{
	if(alive == 1)
	{
			rct.left = X;
...
void CreateBuff(HDC hdc)		//buff erzeugen
{
	buff tempbuff;
	tempbuff.create(hdc);
	tempbuff.update();
	...
	srand( (unsigned)time( NULL ) );
	placebuffvar = rand()%200 + 200;		//alle 200-400 frames
}

void Erase(HDC hdc)			//graphiken entfernen
{
    if(ereasevar == 100)
	{
	RECT rct;
...
void Draw(HDC hdc)			//und neu zeichnen
{
     HDC hdcMemory;
     hdcMemory = CreateCompatibleDC(hdc);
	...
	tempint = P1.lives;				//es folgt lebensanzeige
	lives100 = 0;
	lives10	 = 0;
	lives1   = 0;
	while(tempint >= 100)
...
void Update(HWND hwnd, HDC hdc)		//befehle überprüfen & ausführen
{
